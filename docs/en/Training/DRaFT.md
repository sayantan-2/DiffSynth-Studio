...
| `--draft_truncated_backprop_steps 1` | **K** in DRaFT-K: number of *final* sampler calls retained for autograd. | The preceding `N-K` calls still generate the latent under `torch.no_grad()`; no backward graph is retained for them. `1` is the cheapest, most practical default; `50` with N=50 is original full DRaFT and is much more memory-intensive. |
